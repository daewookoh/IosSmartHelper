...
@implementation FotaASIInputStream

+ (void)initialize
{
	if (self == [FotaASIInputStream class]) {
		readLock = [[NSLock alloc] init];
	}
}

+ (id)inputStreamWithFileAtPath:(NSString *)path request:(FotaASIHTTPRequest *)theRequest
{
	FotaASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
	[theStream setStream:[NSInputStream inputStreamWithFileAtPath:path]];
	return theStream;
}

+ (id)inputStreamWithData:(NSData *)data request:(FotaASIHTTPRequest *)theRequest
{
	FotaASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
...
- (NSInteger)read:(uint8_t *)buffer maxLength:(NSUInteger)len
{
	...
	if ([FotaASIHTTPRequest isBandwidthThrottled]) {
		toRead = [FotaASIHTTPRequest maxUploadReadLength];
		if (toRead > len) {
			toRead = len;
		} else if (toRead == 0) {
			toRead = 1;
	...
	if (rv > 0)
		[FotaASIHTTPRequest incrementBandwidthUsedInLastSecond:rv];
	return rv;
}
