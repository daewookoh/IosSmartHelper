...
static FotaASIAuthenticationDialog *sharedDialog = nil;
BOOL fotaisDismissing = NO;
static NSMutableArray *requestsNeedingAuthentication = nil;

static const NSUInteger FotakUsernameRow = 0;
static const NSUInteger FotakUsernameSection = 0;
static const NSUInteger FotakPasswordRow = 1;
static const NSUInteger FotakPasswordSection = 0;
static const NSUInteger FotakDomainRow = 0;
static const NSUInteger FotakDomainSection = 1;


@implementation FotaASIAutorotatingViewController
...
@interface FotaASIAuthenticationDialog ()
- (void)showTitle;
- (void)show;
...
@implementation FotaASIAuthenticationDialog

#pragma mark init / dealloc

+ (void)initialize
{
	if (self == [FotaASIAuthenticationDialog class]) {
		requestsNeedingAuthentication = [[NSMutableArray array] retain];
	}
}

+ (void)presentAuthenticationDialogForRequest:(FotaASIHTTPRequest *)theRequest
{
...
- (UIViewController *)presentingController
{
	if (!presentingController) {
		presentingController = [[FotaASIAutorotatingViewController alloc] initWithNibName:nil bundle:nil];
...
{
	return [self textFieldInRow:FotakUsernameRow section:FotakUsernameSection];
}

- (UITextField *)passwordField
{
	return [self textFieldInRow:FotakPasswordRow section:FotakPasswordSection];
}

- (UITextField *)domainField
{
	return [self textFieldInRow:FotakDomainRow section:FotakDomainSection];
}
...
- (void)cancelAuthenticationFromDialog:(id)sender
{
	for (FotaASIHTTPRequest *theRequest in [self requestsRequiringTheseCredentials]) {
...
- (NSArray *)requestsRequiringTheseCredentials
{
	...
	for (FotaASIHTTPRequest *otherRequest in requestsNeedingAuthentication) {
...
- (void)presentNextDialog
{
	if ([requestsNeedingAuthentication count]) {
		FotaASIHTTPRequest *nextRequest = [requestsNeedingAuthentication objectAtIndex:0];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (s == FotakUsernameSection && r == FotakUsernameRow) {
		[textField setPlaceholder:@"User"];
	} else if (s == FotakPasswordSection && r == FotakPasswordRow) {
		[textField setPlaceholder:@"Password"];
		[textField setSecureTextEntry:YES];
	} else if (s == FotakDomainSection && r == FotakDomainRow) {
