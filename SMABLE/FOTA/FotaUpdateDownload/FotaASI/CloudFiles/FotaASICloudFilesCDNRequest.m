...
#import "FotaASICloudFilesCDNRequest.h"
#import "FotaASICloudFilesContainerXMLParserDelegate.h"


@implementation FotaASICloudFilesCDNRequest

@synthesize accountName, containerName, xmlParserDelegate;

+ (id)cdnRequestWithMethod:(NSString *)method query:(NSString *)query {
	NSString *urlString = [NSString stringWithFormat:@"%@%@", [FotaASICloudFilesRequest cdnManagementURL], query];
	FotaASICloudFilesCDNRequest *request = [[[FotaASICloudFilesCDNRequest alloc] initWithURL:[NSURL URLWithString:urlString]] autorelease];
...
+ (id)cdnRequestWithMethod:(NSString *)method containerName:(NSString *)containerName {
	NSString *urlString = [NSString stringWithFormat:@"%@/%@", [FotaASICloudFilesRequest cdnManagementURL], containerName];
	FotaASICloudFilesCDNRequest *request = [[[FotaASICloudFilesCDNRequest alloc] initWithURL:[NSURL URLWithString:urlString]] autorelease];
	[request setRequestMethod:method];
	[request addRequestHeader:@"X-Auth-Token" value:[FotaASICloudFilesRequest authToken]];
...
+ (id)containerInfoRequest:(NSString *)containerName {
	FotaASICloudFilesCDNRequest *request = [FotaASICloudFilesCDNRequest cdnRequestWithMethod:@"HEAD" containerName:containerName];
	return request;
}
...
+ (id)listRequest {
	FotaASICloudFilesCDNRequest *request = [FotaASICloudFilesCDNRequest cdnRequestWithMethod:@"GET" query:@"?format=xml"];
	return request;
}
...
+ (id)listRequestWithLimit:(NSUInteger)limit marker:(NSString *)marker enabledOnly:(BOOL)enabledOnly  {
	...
	FotaASICloudFilesCDNRequest *request = [FotaASICloudFilesCDNRequest cdnRequestWithMethod:@"GET" query:query];
	return request;
}
...
+ (id)putRequestWithContainer:(NSString *)containerName {
	FotaASICloudFilesCDNRequest *request = [FotaASICloudFilesCDNRequest cdnRequestWithMethod:@"PUT" containerName:containerName];
	return request;
}
...
+ (id)postRequestWithContainer:(NSString *)containerName {
	FotaASICloudFilesCDNRequest *request = [FotaASICloudFilesCDNRequest cdnRequestWithMethod:@"POST" containerName:containerName];
	return request;
}
