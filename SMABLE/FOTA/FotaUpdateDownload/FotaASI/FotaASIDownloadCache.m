...
#import "FotaASIDownloadCache.h"
#import "FotaASIHTTPRequest.h"
#import <CommonCrypto/CommonHMAC.h>

static FotaASIDownloadCache *sharedCache = nil;

static NSString *sessionCacheFolder = @"SessionStore";
static NSString *permanentCacheFolder = @"PermanentStore";
static NSArray *fileExtensionsToHandleAsHTML = nil;

@interface FotaASIDownloadCache ()
+ (NSString *)keyForURL:(NSURL *)url;
- (NSString *)pathToFile:(NSString *)file;
@end

@implementation FotaASIDownloadCache

+ (void)initialize
{
	if (self == [FotaASIDownloadCache class]) {
...
- (void)updateExpiryForRequest:(FotaASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
...
{
  return [FotaASIHTTPRequest expiryDateForRequest:request maxAge:maxAge];
}

- (void)storeResponseForRequest:(FotaASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	[[self accessLock] lock];
...
- (NSString *)pathToStoreCachedResponseDataForRequest:(FotaASIHTTPRequest *)request
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
...
- (NSString *)pathToStoreCachedResponseHeadersForRequest:(FotaASIHTTPRequest *)request
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
...
- (void)removeCachedDataForRequest:(FotaASIHTTPRequest *)request
{
	[self removeCachedDataForURL:[request url]];
}

- (BOOL)isCachedDataCurrentForRequest:(FotaASIHTTPRequest *)request
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
...
+ (BOOL)serverAllowsResponseCachingForRequest:(FotaASIHTTPRequest *)request
{
...
- (BOOL)canUseCachedDataForRequest:(FotaASIHTTPRequest *)request
{
