...
//                        NSLog(@"jgrighh==%@",[NSData dataWithBytes:testByte length:len]);
        ...
        if (serNum == self.serialNum && epoIndex > 0 && epoFileData.length > 0) {
            [self epoFileWrite];
        }else if (serNum == self.serialNum && epoIndex == 0) {
            //上传完成
            NSLog(@"MTK AGPS update done");
//            if ([self.delegate respondsToSelector:@selector(updateAGPSFileProgress:)]) {
//            }
            epoIndex = 0;
            epoFileData = nil;
        }
...
-(void)checkCmdKeyType:(Byte *)bytes len:(int)len bol:(BOOL)bol {
    ...
    else if(bytes[8]==0x03 && bytes[10]==0x02 && bol)//绑定请求返回命令,验证是否
    {
        ...
        if (error == 0) {
            ...
            [SmaBleMgr.BLdelegate bleDidConnect2];
            [self.delegate_swift connectSuccess];
        }
        else
        {
            [SmaBleMgr.BLdelegate bleDidConnect2];
            [self.delegate_swift connectCancel];
        }
    }
    ...
    else if(bytes[8]==0x02 && bytes[10]==0x2E && bol)//返回07闹钟列表
    {
        ...
        if (array.count == 0) {
            ...
            [self.delegate_swift setAlarm:array];
        }
    }
    ...
    else if(bytes[8]==0x05 && bytes[10]==0x02 && bol)
    {
        ...
        if(bytes[13]==1)
        {
            [self.delegate_swift takePhoto];
        }
        else
        {
            [self.delegate_swift closePhoto];
        }
    }
        ...
        [self.delegate_swift setGoal:array[0]];
    }
        ...
        if (![[array firstObject] isKindOfClass:[NSDictionary class]]) {
            ...
            [self.delegate_swift setSit:info];
        }
    }
        ...
        //[self.delegate_swift findPhone:[NSString stringWithFormat:@"%d",bytes[13]]];
    ...
    else if (bytes[8] == 0x05 && bytes[10] == 0x55 && bol) {
        mode = SPORTV3;
        array = [self manualSportV3WithData:bytes Len:len];
    }
    ...
    if (mode <= 28 && array.count > 0) {
        
        NSLog(@"mode:%d",mode);
        NSLog(@"The content of arry is%@",array);
        
        //if(mode==8 || mode==15)
        if(mode==15)        
        {
            [self.delegate_swift insertHeart:array];
        }
        else if(mode==14)
        {
            [self.delegate_swift insertSport:array];
        }
        else if(mode==16)
        {
            [self.delegate_swift insertSleep:array];
        }
        else if(mode==21)
        {
            [self.delegate_swift insertExercise:array];
        }
        else if(mode==26)
        {
            [self.delegate_swift insertTracker:array];
        }
...
- (void)setGpsWithSpeed:(int)speed elevation:(int)elevation distance:(float)distance{
    ...
    if(self.p && self.Write){
        ...
        [self arrangeBLData:data0 type:@"SET" sendNum:1];
        //        [self.p writeValue:data0 forCharacteristic:self.Write type:CBCharacteristicWriteWithResponse];
        NSLog(@"骑行模式定位信息-------%d, %d, %f",speed,elevation,distance);
    }
}
...
- (void)setGPSWithSpeed:(float)speed Altitude:(int)altitude Distance:(float)distance {
    ...
    if(self.p && self.Write){
        ...
        NSLog(@"骑行模式B3-G定位信息-------%@ -》%@",data01,data02);
...
- (void)requestSportDataV3 {
    Byte results[13];
    if(self.p && self.Write) {
        [SmaBusinessTool getSpliceCmd:0x05 Key:0x54 bytes1:nil len:0 results:results];
        NSData * data0 = [NSData dataWithBytes:results length:13];
        [self arrangeBLData:data0 type:@"GET" sendNum:1];
        //[self.p writeValue:data0 forCharacteristic:self.Write type:CBCharacteristicWriteWithResponse];
    }
...
- (void)analySwitchsWithdata:(NSData *)data replace:(int)number{
    ...
    NSLog(@"swichnum==%d",_switchNumber);
    Byte tempbyt[1];
    if (_switchNumber == 111) {
        tempbyt[0] = (Byte)(_switchNumber&0xff);
    }else if (_switchNumber == 71) {
        tempbyt[0] = (Byte)(_switchNumber&0xff);
    }
    else {
        tempbyt[0]=(Byte)((_switchNumber&0xff)+ 0x30);
    }
    [self.BinArr addObject:[NSData dataWithBytes:tempbyt length:1]];
    int datNum = [NSString stringWithFormat:@"%lu",(unsigned long)data.length].intValue/128;
    Byte cmd_data[128];
...
- (NSMutableArray *)analysisLongTimeData:(Byte *)bytes len:(int)len{
    ...
    if (burnthe_t.data != 0) {
        SmaSeatInfo *info = [SmaSeatInfo share];
...
- (NSMutableArray *)manualSportV2RunWithData:(Byte *)byte Len:(int)len{
    ...
    for (int i = 0; i<dataLen/34; i++) {
        ...
        elevation = [NSString stringWithFormat:@"%.2f",byte[25+34*i]*pow(16, 2) + byte[26+34*i]];
        pressure = [NSString stringWithFormat:@"%.2f",byte[27+34*i]*pow(16, 2) + byte[28+34*i]];
        ...
        distance = [NSString stringWithFormat:@"%.2f",(float)(byte[35+34*i]*pow(16, 6) + byte[36+34*i]*pow(16, 4) + byte[37+34*i]*pow(16, 2) + byte[38+34*i])]; //最小单位米
        cal = [NSString stringWithFormat:@"%.0f",byte[39+34*i]*pow(16, 6) + byte[40+34*i]*pow(16, 4) + byte[41+34*i]*pow(16, 2) + byte[42+34*i]];
        speed = [NSString stringWithFormat:@"%.2f",byte[43+34*i]*pow(16, 2) + byte[44+34*i]];
        pace = [NSString stringWithFormat:@"%.2f",byte[45+34*i]*pow(16, 2) + byte[46+34*i]];
...
//运动数据V3返回
static NSMutableArray *SportArrV3;
- (NSMutableArray *)manualSportV3WithData:(Byte *)byte Len:(int)len {
    if (!SportArrV3) {
        SportArrV3 = [NSMutableArray array];
    }
    NSMutableArray *allSpArr=[NSMutableArray array];
    NSTimeZone* GTMzone = [NSTimeZone timeZoneForSecondsFromGMT:0];//解决不同时令相差1小时问题
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    ...
    int dataLen = byte[11]*pow(16, 2) + byte[12];
    if (dataLen == 0) {
        if (SportArrV3.count == 0) {
            NSDictionary *dic = [[NSDictionary alloc] initWithObjectsAndKeys:@"NODATA",@"NODATA", nil];
            [allSpArr addObject:dic];
        }
        else{
            allSpArr = [SportArrV3 mutableCopy];
            [SportArrV3 removeAllObjects];
            SportArrV3 = nil;
        }
    }
    NSString *step;
    ...
    for (int i = 0; i<dataLen/16; i++) {
        time = byte[13+16*i]*pow(16, 6) + byte[14+16*i]*pow(16, 4) + byte[15+16*i]*pow(16, 2) + byte[16+16*i];
        mode = [NSString stringWithFormat:@"%d",byte[17+16*i]];
        step = [NSString stringWithFormat:@"%.0f",byte[18+16*i]*pow(16, 4) + byte[19+16*i]*pow(16, 2) + byte[20+16*i]];
        calorie = [NSString stringWithFormat:@"%.2f",(float)(byte[21+16*i]*pow(16, 6) + byte[22+16*i]*pow(16, 4) + byte[23+16*i]*pow(16, 2) + byte[24+16*i])/10000];   //kcal
        distance = [NSString stringWithFormat:@"%.2f",(byte[25+16*i]*pow(16, 6) + byte[26+16*i]*pow(16, 4) + byte[27+16*i]*pow(16, 2) + byte[28+16*i])/10000];  //单位米
        NSDate *date = [[NSDate alloc] initWithTimeInterval:time sinceDate:[formatter dateFromString:@"20000101000000"]];
        NSString *timestr = [formatter stringFromDate:date];
        NSDictionary *dic = [[NSDictionary alloc] initWithObjectsAndKeys:timestr,@"DATE",step,@"STEP",mode,@"MODE",calorie,@"CAL",distance,@"DISTANCE", nil];
        NSLog(@"dic==%@",dic);
        [SportArrV3 addObject:dic];

        //此处可以考虑写入Apple Health

        if (i==dataLen/16-1&&dataLen==240) {
            [self requestSportDataV3];
        }
        if (i==dataLen/16-1&&dataLen!=240){
            allSpArr = [SportArrV3 mutableCopy];
            [SportArrV3 removeAllObjects];
...
-(NSMutableArray *)analysisCuffSleepData:(Byte *)byte len:(int)len{
    ...
    NSInteger time;
    /*NSDate *startDates = nil; //sven add
    NSDate *endDates = nil; //sven add */
...
- (NSMutableArray *)analysisCuffcylingData:(Byte *)byte len:(int)len{
    ...
    for (int i = 0; i<dataLen/7; i++) {
        ...
        else if (time == 4294967294) { //0xfffffffe
            state = @"34";
            timestr = [formatter stringFromDate:dateNow];
        }
...
- (NSMutableArray *)analysisGpsData:(Byte *)byte len:(int)len{
    ...
    for (int i = 0; i<dataLen/16; i++) {
        ...
        memcpy(&lo, &byte[21+16*i], 4);
        memcpy(&la, &byte[25+16*i], 4);

        time = byte[13+16*i]*pow(16, 6) + byte[14+16*i]*pow(16, 4) + byte[15+16*i]*pow(16, 2) + byte[16+16*i];
        runTime =  byte[17+16*i]*pow(16, 2) + byte[18+16*i];   //运动类型
        order =  byte[19+16*i]*pow(16, 2) + byte[20+16*i];
        if (order > 8000) {
            continue;
        }

        lati = [self byteToFloat:la];
        longi = [self byteToFloat:lo];
        //        NSString *tt = @"2000-01-01 00:00:00";
        //        NSDateFormatter *fmt = [[NSDateFormatter alloc] init];
        //        fmt.dateFormat = @"yyyy-MM-dd HH:mm:ss";
        //        NSDate *ttdate = [fmt dateFromString:tt];
        //        NSTimeInterval sinet = [ttdate timeIntervalSince1970];  //秒
        //        NSDate *timeData = [NSDate dateWithTimeIntervalSince1970:sinet+time];
        //        NSLog(@"time==%ld--(%d%d%d%d)",time,byte[13+16*i],byte[14+16*i],byte[15+16*i],byte[16+16*i]);
        //        NSLog(@"timeDate==%@",timeData);

        NSDate *date = [[NSDate alloc] initWithTimeInterval:time sinceDate:[formatter dateFromString:@"20000101000000"]];
        NSString *timestr = [formatter stringFromDate:date];
        NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:timestr,@"DATE",[NSString stringWithFormat:@"%f",longi],@"LONGITUDE",[NSString stringWithFormat:@"%f",lati],@"LATITUDE",[NSString stringWithFormat:@"%ld",(long)order],@"ALTITUDE",/*[NSString stringWithFormat:@"%ld",(long)runTime],@"MODE",*/ nil];  //"@"ORDER",
        NSLog(@"定位数据==%@",dic);
...
- (void)writeFirmware:(int)NUM{
    ...
    if (NUM < self.BinArr.count) {
        ...
        if (modNum > 0) {
            ...
            NSLog(@"writeFirmware write==%@",[NSData dataWithBytes:result_endsub length:modNum]);
    ...
    NSLog(@"writeFirmware update progress ==%@",progress);
...
- (void)getDeviceRequestData:(Byte *)bytes len:(int)len {
    ...
    if (bytes[13] == 0x00 && bytes[14] == 0x01) {
        [self updateEPOFileForAGPS];
    }else if (bytes[13] == 0x00 && bytes[14] == 0x02) {
        //ublox星历文件获取指令
        [self updateUbloxFileForAGPS];
    }
}

- (void)updateUbloxFileForAGPS{
    //手环获取AGPS文件
    NSFileManager *fileManage = [NSFileManager defaultManager];
    NSString *documentsPath = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject];
    NSString *bundlePath = [documentsPath stringByAppendingPathComponent:@"current_1d.alp"];//sven update current_3d.alp->current_1d.alp
    [fileManage removeItemAtPath:bundlePath error:nil];
    [FotaAFNetRequestManager FotaDownloadFileWithURL:@"https://alp.u-blox.com/current_1d.alp" Success:^(id responseObject) {
        NSLog(@"response==%@",responseObject);
        if (responseObject) {
            NSURL *url = (NSURL *)responseObject;
            NSString *filePath = url.absoluteString;
            NSData *data = [NSData dataWithContentsOfFile:[filePath substringFromIndex:7]];
            [SmaBleSend analySwitchsWithdata:data replace:71];
...
- (void)updateEPOFileForAGPS {

    //mtk2503 epo辅助定位文件获取请求；
    NSFileManager *fileManage = [NSFileManager defaultManager];
    NSString *documentsPath = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject];
    NSString *bundlePath = [documentsPath stringByAppendingPathComponent:@"EPO_GR_3_1.DAT"];
    [fileManage removeItemAtPath:bundlePath error:nil];

    [FotaAFNetRequestManager FotaDownloadFileWithURL:@"http://wepodownload.mediatek.com/EPO_GR_3_1.DAT" Success:^(id responseObject) {
...
static int epoIndex = -1;

- (void)updataEPOFileWithPath:(NSURL *)filePath  {

    [SmaBleSend setSystemTime];   //先同步时间

    NSString *file = filePath.absoluteString;
    NSLog(@"%@",[file substringFromIndex:7]);
    epoFileData = [NSData dataWithContentsOfFile:[file substringFromIndex:7]];

    int file_len = (int)epoFileData.length;
    ...
    NSDate *now = [NSDate dateWithTimeIntervalSince1970:sun];    //时间
    //时间转日期
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    [formatter setDateFormat:@"yyyy:MM:dd:HH:mm:ss"];
    NSString *dateString  = [NSString stringWithFormat:@"%@\t",[formatter stringFromDate:now]];
    NSLog(@"dateAtring==%@",dateString);
    const char *timeStr = [dateString cStringUsingEncoding:NSASCIIStringEncoding];
    size_t timeStrlen = strlen(timeStr);

    int countepo = file_len % 2033 == 0 ? file_len /2033 : file_len / 2033 + 1;

    int flag = 1;
    memcpy(flagbuf, &flag, 2);   //文件flag
    Byte flagBB[2];
    for (int k=0; k<2; k++) {
    ...
    Byte results[10+timeStrlen+13];
    int resultCount = (int)(timeStrlen+23);
    Byte valueBuf[10 +timeStrlen];
    ...
    for (int i=0; i<10+timeStrlen; i++) {
        if (i < 2) {
            valueBuf[i] = index[i];
        }else if (i >= 2 && i < 6) {
            valueBuf[i] = sizeB[i-2];
        }else if (i >= 6 && i < 8) {
            valueBuf[i] = flagBB[i-6];
        }else if (i >= 8 && i < 10) {
            valueBuf[i] = sumepo[i-8];
        }else if (i >= 10 && i < 10+timeStrlen) {
            valueBuf[i] = timeStr[i-10];
        }
    ...
    epoIndex = countepo;
    //先发送
    [SmaBusinessTool getSpliceCmd:0x05 Key:0x60 bytes1:valueBuf len:(int)(10+timeStrlen) results:results];
    if(self.p&& self.Write)
    {
...
- (void)epoFileWrite {
    if (epoFileData && epoFileData.length > 0) {
        ...
        NSData *indexData = [[NSData alloc] initWithBytes:index length:2];

        NSMutableData *epoMData = [[NSMutableData alloc] initWithData:indexData];

        int cur_count = (countepo - epoIndex);
        int transfer_len = 2033;
        if (cur_count == (countepo - 1)) {
            //最后一包
            transfer_len = epoFileData.length - (countepo - 1)*2033;
        }
        NSData *cur_data = [epoFileData subdataWithRange:NSMakeRange(2033*(cur_count), transfer_len)];

        [epoMData appendData:cur_data];

        Byte eporesults[epoMData.length+13];
        int eporesultCount = epoMData.length+13;


        Byte *cmdData = [SmaBusinessTool getEPOFileCmd:0x05 Key:0x60 len:transfer_len+2 data:epoMData];
        int cmd_len = transfer_len + 15;

        if (self.p && self.Write) {


            int degree = cmd_len / 180;
            int surplus = cmd_len % 180;
            if(surplus>0)
                degree=degree+1;
            for (int k=0; k<degree; k++) {
                int alen=(((degree-1)==k) ? (surplus > 0 ? surplus : 180) : 180);
                Byte arr[alen];
                memcpy(&arr,&cmdData[0+(180*k)],alen);//拷贝到对应的Byte 数组中
                NSData * data0 = [NSData dataWithBytes:arr length:(((degree-1)==k)? (surplus > 0 ? surplus : 180) : 180)];
                [self arrangeBLData:data0 type:@"SET" sendNum:degree];
                //                        NSLog(@"结束%@",data0);
                //                        [self.p writeValue:data0 forCharacteristic:self.Write type:CBCharacteristicWriteWithResponse];
            }
        ...
        float pp = (float) cur_count / countepo;
        NSString *progress = [NSString stringWithFormat:@"%.2f", pp];
        NSLog(@"MTK AGPS update progress %@", progress);
//        if ([self.delegate respondsToSelector:@selector(updateAGPSFileProgress:)]) {
//            float pp = (float) cur_count / countepo;
//            NSString *progress = [NSString stringWithFormat:@"%.2f", pp];
//            NSLog(@"AGPS update progress %@", progress);
//        }
...
- (void)setWatchFace:(int)flag faceId:(int)faceId index:(int)index {

    NSData *reader=[NSData dataWithContentsOfFile:[[NSBundle mainBundle] pathForResource:[NSString stringWithFormat:@"bp%d",faceId] ofType:@"bin"]];
    int file_len = (int)reader.length;
    epoFileData = reader;
    ...
    NSString *dateString  = [NSString stringWithFormat:@"%d\t%d\t",faceId,index];
    const char *indexStr = [dateString cStringUsingEncoding:NSASCIIStringEncoding];
    size_t indexStrlen = strlen(indexStr);

    Byte results[10+indexStrlen+13];
    int resultCount = indexStrlen+23;
    Byte valueBuf[10 +indexStrlen];
    ...
    for (int i=0; i<10+indexStrlen; i++) {
        if (i < 2) {
            valueBuf[i] = index0[i];
        }else if (i >= 2 && i < 6) {
            valueBuf[i] = sizeB[i-2];
            ...
            valueBuf[i] = sumepo[i-8];
        }else if (i >= 10 && i < 10+indexStrlen) {
            valueBuf[i] = indexStr[i-10];
        }
    ...
    epoIndex = countepo;
    //先发送
    [SmaBusinessTool getSpliceCmd:0x05 Key:0x60 bytes1:valueBuf len:(10+indexStrlen) results:results];
    if(self.p&& self.Write)
    {
        int degree = resultCount/20;
        int surplus = resultCount%20;
...
- (void)setBrightInfo:(SMARightScreenInfo *)info {
    NSArray *array = [[self toBinarySystemWithDecimalSystem:info.repeatWeek] componentsSeparatedByString:@","];
    burntheplanks_week_union_t week_t1;
    week_t1.bit_week.monday=([array[0] isEqualToString:@"0"])?0x00:0x01;
    week_t1.bit_week.tuesday=([array[1] isEqualToString:@"0"])?0x00:0x01;
    ...
    Byte buf[5];
    buf[0] = 0x00;
    buf[1] = ([info.isOpen intValue]&0xff);
    buf[2] = [info.beginTime intValue]&0xff;
    buf[3] = [info.endTime intValue]&0xff;
    buf[4] = week_t1.week;

    Byte result[18];
    if(self.p && self.Write){
        [SmaBusinessTool getSpliceCmd:0x02 Key:0x6B bytes1:buf len:5 results:result];
        NSData * data0 = [NSData dataWithBytes:result length:18];
        [self arrangeBLData:data0 type:@"SET" sendNum:1];
        //        [self.p writeValue:data0 forCharacteristic:self.Write type:CBCharacteristicWriteWithResponse];
    }
}
